...
- `/랭킹` — 전용 `rankings.json`만 사용하는 독립 시스템. 길드 안에서도 **채널별로**
  유저마다 `{ name, sum, rollCount, lastRolledAt }`를 저장하고, 같은 길드의 모든 채널
  기록을 합산한 **서버 전체 랭킹**도 함께 표시한다. 저장·조회 키는 항상 길드 ID 아래여서
  다른 서버의 기록은 합산하거나 볼 수 없다. `/roll`의 주사위 식 결과를 자동 저장·참조하며,
  정수만 입력한 `/roll`은 제외한다. 순위는 **누적 합계**(모든 `/roll` 결과의 합)가 큰
  순서이고, 합계가 같으면 굴림 횟수가 적은 쪽이 앞선다. 표시는 한 줄에
  순위·이름·합계·횟수만 두고 개별 굴림 내용은 보여주지 않는다. `sum` 필드가 없는 옛
  최고·최저 방식 레코드는 합계를 복원할 수 없어 다음 굴림부터 0에서 새로 시작한다.
