...
- `/랭킹` — 전용 `rankings.json`만 사용하는 독립 시스템. 길드 안에서도 **채널별로**
  ...
  정수만 입력한 `/roll`은 제외한다. 랭킹은 **나온 합계**를 우선 비교하고, 합계가
  같을 때만 **주사위의 가능한 최댓값**이 큰 쪽을 우선한다. 나온 값 ÷ 가능한 최댓값 비율은
  결과 설명용으로 표시할 뿐 순위를 단독으로 결정하지 않으므로 `1d2`의 2가 자동 1위가 되지
  않는다. 랭킹 저장 오류는 `/roll`의 결과·응답에 영향을 주지 않으며, `rankings.json`은
  `DATA_DIR` 아래에 저장된다.
