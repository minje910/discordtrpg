...
const commands = [
  ...
  new SlashCommandBuilder().setName('관리자').setDescription('관리자 모드 (지정된 계정만 사용 가능)')
    ...
    .addSubcommand(s => s.setName('주사위고정').setDescription('주사위 굴림 결과를 지정한 값으로 고정합니다')
      .addIntegerOption(o => o.setName('값').setDescription('나올 값 (면수보다 크면 최대값으로 맞춰짐)').setRequired(true))
      .addIntegerOption(o => o.setName('횟수').setDescription('적용할 주사위 개수 (기본 1, 0이면 해제할 때까지 무제한)'))
      .addStringOption(o => o.setName('대상').setDescription('누구의 굴림에 적용할지 (기본: 나만)')
